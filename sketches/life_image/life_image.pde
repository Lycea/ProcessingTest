...
class Pix
{
	...
	update()
	{
		...
		{
			if(grid[this.neigh[idx][1]][this.neigh[idx][0]].state == 1)
			{
				allive+=1
				cols+=grid[this.neigh[idx][1]][this.neigh[idx][0]].col
			}
		}
		...
		if(this.state == 1)
		{
			...
			this.next_col = (this.col+cols)/(allive+1)
		}
		else
		{
		...
		if(this.state != this.next || this.state ==1){ to_update.push([this.x,this.y])}
	}
