...
function draw()
{
	...
	/*
	if(dist1d(second(),last_process)<1)
	{
		if(once == false)
		{
			//print(len_array)
		...
		//print(dist1d(second(),last_process))
		return
	}
	once = false
	*/
	
	for(var len_array; len_array >0;len_array-- )
	{
		var point_ = array_points[len_array-1]
		...
		array_points.splice(len_array-1,1)
	}
	last_process = second()
